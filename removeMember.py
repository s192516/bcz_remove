...
class RemoveMember:
    ...
    @print_function_name
    def rename_bookname(self, member):
        bookname = member['bookName']
        if "年级起" in bookname:
            bookname = bookname.replace("年级起", "")
        member['bookName'] = bookname
    ...
    def get_whitelist_blacklist(self):
        whitelist, whitelist_name, new_member = self.sqlite.get_whitelist()
        blacklist = self.config.get_blacklist()

        for groupname, username in whitelist_name.items():
            if len(new_member[groupname]) != 0:
                self.logger.info(f" {groupname} 白名单成员共: {len(username)}人, 新增{len(new_member[groupname])}人, 名单为{new_member[groupname]}",)
                print(f" {groupname} 白名单成员共: {len(username)}人, 新增{len(new_member[groupname])}人, 名单为{new_member[groupname]}")
